  ...
  return (
    <div className="dashboard" style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      <div className="glass-card" style={{ marginBottom: '2rem', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '4rem', fontWeight: 'bold', color: percentage >= 50 ? '#4caf50' : '#ff4a4a', marginBottom: '1rem' }}>
            ...
            <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {q.options.map((opt, oIndex) => {
                ...
                if (oIndex === q.correctOptionIndex) {
                  bgColor = 'rgba(76, 175, 80, 0.2)'; // Green for correct
                  border = '1px solid #4caf50';
                } else if (oIndex === q.userSelectedOptionIndex && !q.isCorrect) {
                  bgColor = 'rgba(244, 67, 54, 0.2)'; // Red for wrong selected
                ...
                return (
                  <div key={oIndex} style={{ padding: '0.8rem', background: bgColor, border, borderRadius: '4px', display: 'flex', justifyContent: 'space-between' }}>
                    <span>{opt}</span>
                    {oIndex === q.userSelectedOptionIndex && <span>(Your Answer)</span>}
                    {oIndex === q.correctOptionIndex && <span>(Correct Answer)</span>}
                  </div>
                );
              })}
            </div>
